...
const DisenadorCanales = () => {
  ...
  const [tituloHoja, setTituloHoja] = useState(
    "DISEÑO PARA FABRICACIÓN DE CANALES CUBIERTA BODEGA",
  );
  ...
  const [inicioEjeNum, setInicioEjeNum] = useState("1");
  const [muroIzquierdo, setMuroIzquierdo] = useState(false);
  const [muroDerecho, setMuroDerecho] = useState(false);
  ...
  const tieneP1 = pliegues.some(
    (p) => p.detalle === "Pestaña" || p.detalle === "Pestaña Recuperada",
  );
  const tieneUltima = pliegues.some(
    (p) =>
      p.detalle === "Pestaña Cubierta" || p.detalle === "Cubierta Recuperada",
  );
  ...
  useEffect(() => {
    ...
    const inicializarCol = (id) => {
      if (!nuevaConfig[id]) {
        nuevaConfig[id] = {
          unirDerecha: false,
          longitudDerecha: 2000,
          pendiente: 0,
          planaCentro: 0,
          planaDerecha: 0,
          planaIzquierda: 0,
          soscoCentro: false,
          soscoIzquierdo: false,
          soscoDerecho: false,
          dosSoscos: false,
          diametroSosco: '4"',
          listaTraslapos: [],
    ...
    for (let i = 0; i < numColumnas; i++) inicializarCol(i);
    if (muroIzquierdo) inicializarCol("muro-izq");
    if (muroDerecho) inicializarCol("muro-der");

    if (huboCambio) setConfigColumnas(nuevaConfig);
  }, [numColumnas, muroIzquierdo, muroDerecho]);
  ...
  useEffect(() => {
    ...
    hitos.forEach((hito) => {
      if (
        !nuevaMatriz[hito.keyHash] ||
        nuevaMatriz[hito.keyHash].length !== pliegues.length
      ) {
  ...
  }, [pliegues, numColumnas, configColumnas, muroIzquierdo, muroDerecho]);
  ...
  const cargarListaPlantillas = async () => {
    ...
    if (!error) {
      setListaPlantillas(
        data.filter((p) => p.tramos?.modulo === "canales_plantilla"),
      );
    }
  };
  ...
  const generarColumnasLineales = () => {
    ...
    const idInicial = muroIzquierdo ? "muro-izq" : 0;
    const idFinal = muroDerecho ? "muro-der" : numColumnas - 1;

    const colInicialConfig = configColumnas[idInicial] || {};
    const colFinalConfig = configColumnas[idFinal] || {};

    const tienePlanaIzquierda =
      parseFloat(colInicialConfig.planaIzquierda) > 0 ||
      parseFloat(colInicialConfig.planaCentro) > 0;
    const tienePlanaDerecha =
      parseFloat(colFinalConfig.planaDerecha) > 0 ||
      parseFloat(colFinalConfig.planaCentro) > 0;

    const inicioRealX = tienePlanaIzquierda ? 15 + anchoVisualPartePlana : 15;
    const finRealX = tienePlanaDerecha
      ? anchoMaxSvn - 15 - anchoVisualPartePlana
      : anchoMaxSvn - 15;
    ...
    const inicioEjeReal = parseInt(inicioEjeNum) || 1;
    for (let i = 0; i < numColumnas; i++) {
      const numeroNormal = inicioEjeReal + i;
      const numeroInvertido = inicioEjeReal + (numColumnas - 1 - i);
      const numeroMostrado = invertirNumeracion
        ? numeroInvertido
        : numeroNormal;
      elementos.push({
        id: i,
    ...
    // 💡 MATEMÁTICA EXACTA DE ESPACIOS
    // El espacio entre ejes vale "1". El espacio Muro-Eje vale "0.5" (exactamente la mitad).
    const factorMuro = 0.5;
    const totalIntervalos =
      numColumnas -
      1 +
      (muroIzquierdo ? factorMuro : 0) +
      (muroDerecho ? factorMuro : 0);
    const D = totalIntervalos > 0 ? anchoUtilCalculado / totalIntervalos : 0;

    let currentX = inicioRealX;

    elementos.forEach((el, index) => {
      cols.push({ ...el, x: currentX });

      // Verificamos de qué tamaño será el salto hacia el siguiente elemento
      if (index < elementos.length - 1) {
        const nextEl = elementos[index + 1];
        if (el.esMuro || nextEl.esMuro) {
          currentX += D * factorMuro; // Salto corto (50%)
        } else {
          currentX += D; // Salto normal entre ejes
        }
      }
    });
      ...
      if (cIdx < columnasCalculadas.length - 1) {
        ...
        listaT.forEach((_, tIdx) => {
          hitos.push({
            ...
            label: `Traslapo T${tIdx + 1} de ${
              col.esMuro ? "Muro" : "Eje " + col.numero
            }`,
          });
        });
      }
  ...
  const nuevoDocumento = () => {
    if (
      window.confirm("¿Crear un nuevo diseño? Se limpiará toda la pantalla.")
    ) {
      setProjectId(null);
      setConfigColumnas({});
      ...
      setInicioEjeNum("1");
      setMuroIzquierdo(false);
      setMuroDerecho(false);
      ...
      setNombreDisenador("");
      setFechaDiseno(new Date().toISOString().split("T")[0]);
      setPliegues([
        { longitud: 50, angulo: 0, detalle: "Pestaña" },
        { longitud: 220, angulo: 90, detalle: "Aleta Izq" },
        { longitud: 400, angulo: 0, detalle: "Fondo" },
  ...
  const duplicarEstructuraViga1 = () => {
    ...
    if (window.confirm("¿Copiar configuración de Viga 1 a toda la canal?")) {
      ...
      for (let i = 1; i < numColumnas; i++) {
        nuevaConfig[i] = {
          ...configBase,
          listaTraslapos: configBase.listaTraslapos
            ? configBase.listaTraslapos.map((t) => ({ ...t }))
            : [],
        };
      }
  ...
  const restaurarPlantillaEstandar = () => {
    if (
      window.confirm(
        "¿Seguro que deseas restaurar la canal a las 5 caras estándar?",
      )
    ) {
  ...
  const agregarPliegue = () => {
    ...
    setPliegues([
      ...pliegues,
      {
        longitud: parseFloat(nuevoLongitud) || 0,
        angulo: parseFloat(nuevoAngulo) || 0,
  ...
  const toggleCaraExtrema = (tipo) => {
    ...
    if (tipo === "P1") {
      if (tieneP1) {
        copiaPliegues.shift();
        Object.keys(copiaMatriz).forEach((key) => {
          if (Array.isArray(copiaMatriz[key])) copiaMatriz[key].shift();
        });
        ...
        Object.keys(copiaMatriz).forEach((key) => {
          if (Array.isArray(copiaMatriz[key]))
            copiaMatriz[key].unshift({ ...nuevaP1 });
        });
      ...
      if (tieneUltima) {
        copiaPliegues.pop();
        Object.keys(copiaMatriz).forEach((key) => {
          if (Array.isArray(copiaMatriz[key])) copiaMatriz[key].pop();
        });
        ...
        const nuevaUlt = {
          longitud: 40,
          angulo: 0,
          detalle: "Pestaña Cubierta",
        };
        copiaPliegues.push(nuevaUlt);
        Object.keys(copiaMatriz).forEach((key) => {
          if (Array.isArray(copiaMatriz[key]))
            copiaMatriz[key].push({ ...nuevaUlt });
        });
  ...
  const modificarPliegueDeHito = (index, campo, valor) => {
    const copiaMatriz = { ...matrizPliegues };
    if (!copiaMatriz[hitoSeleccionado])
      copiaMatriz[hitoSeleccionado] = pliegues.map((p) => ({ ...p }));
    copiaMatriz[hitoSeleccionado][index] = {
      ...copiaMatriz[hitoSeleccionado][index],
      [campo]: valor,
    };
    setMatrizPliegues(copiaMatriz);
  ...
  const duplicarMedidasEnTodosLosHitos = () => {
    if (
      window.confirm(
        "¿Copiar el patrón de medidas de la Viga 1 y sus traslapos a toda la canal?",
      )
    ) {
      const copiaMatriz = { ...matrizPliegues };
      const medidasViga1 =
        copiaMatriz["viga-0"] || pliegues.map((p) => ({ ...p }));
      ...
      for (let t = 0; t < numTraslaposV1; t++) {
        ...
        traslaposViga1.push(
          copiaMatriz[keyT] || pliegues.map((p) => ({ ...p })),
        );
      }
  ...
  const plieguesHitoActual =
    matrizPliegues[hitoSeleccionado] || pliegues.map((p) => ({ ...p }));
  const desarrolloHitoActual = plieguesHitoActual.reduce(
    (s, p) => s + (parseFloat(p.longitud) || 0),
    0,
  );
  const alertaDesarrollo =
    desarrolloHitoActual < 1000
  ...
  const guardarComoDB = async () => {
    ...
    setMensaje(
      res.error
        ? "❌ Error al guardar como"
        : "✅ Archivo nuevo creado exitosamente",
    );
  };

  const guardarComoPlantilla = async () => {
    const nombrePlantilla = window.prompt(
      "Ingrese el nombre para esta Plantilla Estándar (Ej: Canal Tipo 1):",
    );
  ...
  const cargarPlantillaEspecifica = (idPlantilla) => {
    ...
    setNombreDisenador(info.nombreDisenador || "");
    setFechaDiseno(info.fechaDiseno || new Date().toISOString().split("T")[0]);

    setNumColumnasInput((info.numColumnas || 6).toString());
    setNumColumnas(info.numColumnas || 6);
    setInicioEjeNum(info.inicioEjeNum || "1");
    setMuroIzquierdo(!!info.muroIzquierdo);
    setMuroDerecho(!!info.muroDerecho);
    ...
    setMensaje(
      "✅ Plantilla estándar cargada. Dale a 'Guardar' para crear tu proyecto.",
    );
  };

  const eliminarPlantilla = async () => {
    if (!plantillaSeleccionada) {
      alert("Por favor, seleccione una plantilla de la lista primero.");
      return;
    }
    const plantilla = listaPlantillas.find(
    ...
    if (
      ...
      setMensaje("Eliminando plantilla...");
      const { error } = await supabase
        .from("diseños_canales")
        .delete()
        .eq("id", plantillaSeleccionada);
      if (!error) {
        setMensaje("✅ Plantilla eliminada exitosamente.");
        setPlantillaSeleccionada("");
        cargarListaPlantillas();
      } else {
        setMensaje("❌ Error al eliminar la plantilla.");
      }
    }
  };
  ...
  const abrirModalCarga = async () => {
    ...
    const { data, error } = await supabase
      ...
      .order("ultima_actualizacion", { ascending: false });
    if (!error)
      setListaProyectos(data.filter((p) => p.tramos?.modulo === "canales"));
  ...
  const actualizarPropiedadColumna = (colId, campo, valor) => {
    ...
    if (campo === "longitudDerecha" && parseFloat(valor) > 6000)
      valorFinal = "6000";
    setConfigColumnas((prev) => ({
      ...prev,
      [colId]: { ...prev[colId], [campo]: valorFinal },
    }));
  };
  ...
  const actualizarPropiedadTraslapo = (colId, index, campo, valor) => {
    let valorFinal = valor;
    if (
      (campo === "longitud" || campo === "longitudCierre") &&
      parseFloat(valor) > 6000
    )
      valorFinal = "6000";
    setConfigColumnas((prev) => {
      ...
      return {
        ...prev,
        [colId]: { ...prev[colId], listaTraslapos: listaOriginal },
      };
    });
  };
  ...
  const fuentes =
    numColumnas <= 6
      ? { cotas: 9.5, soscos: 8, columnas: 13 }
      : numColumnas <= 12
        ? { cotas: 8, soscos: 7, columnas: 11 }
        : { cotas: 6.5, soscos: 5.5, columnas: 9 };
  ...
  const datosGeometria = (() => {
    ...
    columnasCalculadas.forEach((col, index) => {
      ...
      puntosEstructura[col.id] = {
        xCol: col.x,
        yInicioColumna: yActual,
        lineas: [],
        cotas: [],
        soscosSVG: [],
        ejesTraslapos: [],
      ...
      const renderizarSoscosIndependientes = (y) => {
        ...
        const crearAux = (xPos, k) => (
          <g key={k}>
            ...
            />
            <text
              x={xPos + anchoSosco / 2 + 2}
              y={y + altoSosco / 2 + 3}
              fontSize={fuentes.soscos}
              fill="red"
              fontWeight="bold"
            >
              {diam}
        ...
        const xCentro = col.x;
        const xIzq =
          parseFloat(config.planaIzquierda) > 0
            ? col.x - anchoFijoPlana / 2
            : col.x - 12;
        const xDer =
      ...
      puntosEstructura[col.id].soscosSVG.push(
        ...renderizarSoscosIndependientes(yActual),
      );

      if (parseFloat(config.planaIzquierda) > 0) {
        puntosEstructura[col.id].lineas.push({
          x1: col.x - anchoFijoPlana,
          y1: yActual,
          x2: col.x,
          y2: yActual,
          color: "blue",
        ...
        puntosEstructura[col.id].cotas.push({
          x: col.x - anchoFijoPlana / 2,
          y: yActual + 28,
          texto: config.planaIzquierda,
        });
      }
      if (parseFloat(config.planaCentro) > 0) {
        puntosEstructura[col.id].lineas.push({
          x1: col.x - anchoFijoPlana / 2,
          y1: yActual,
          x2: col.x + anchoFijoPlana / 2,
          y2: yActual,
          color: "blue",
          width: 2,
        });
      ...
      if (index < columnasCalculadas.length - 1) {
        ...
        let finX =
          sigCol.x -
          (parseFloat(configColumnas[sigCol.id]?.planaIzquierda) > 0
            ? anchoFijoPlana
            : parseFloat(configColumnas[sigCol.id]?.planaCentro) > 0
              ? anchoFijoPlana / 2
              : 0);
        ...
        if (listaT.length > 0) {
          ...
          listaT.forEach((traslapo, idx) => {
            ...
            const dY =
              (xSiguiente - xCursor) * Math.sin((pend * Math.PI) / 180);
            ...
            puntosEstructura[col.id].cotas.push({
              x: (xCursor + xSiguiente) / 2,
              y: yActual + dY / 2 + desfaseY,
              texto: traslapo.longitud,
            });
            puntosEstructura[col.id].ejesTraslapos.push({
              x: xSiguiente,
              clickY: yFinal,
              index: idx,
            });
          ...
          if (ultimoT.conectarA === "columna") {
            ...
            const dYCierre =
              (finX - xCursor) * Math.sin((pendCierre * Math.PI) / 180);
            puntosEstructura[col.id].lineas.push({
              x1: xCursor,
              y1: yActual,
              x2: finX,
            ...
            puntosEstructura[col.id].cotas.push({
              x: (xCursor + finX) / 2,
              y: yActual + dYCierre / 2 + desfaseY,
              texto: ultimoT.longitudCierre,
            });
  ...
  const colActual =
    menuFlotante.columnaId !== null
      ? configColumnas[menuFlotante.columnaId]
      : null;
  const traslapoActual =
    menuFlotante.esTraslapo &&
    colActual &&
    colActual.listaTraslapos &&
    menuFlotante.traslapoIndex !== null
  ...
  return (
    <div
      ...
      onClick={() => setMenuFlotante({ ...menuFlotante, visible: false })}
    >
      <style>
        {`
          @media print {
      ...
      >
        ...
        >
          <label style={{ ...labelTitleStyle, color: "#6b21a8" }}>
            PLANTILLAS ESTÁNDAR:
          </label>
        ...
        <label style={labelTitleStyle}>DISEÑADOR:</label>
        <input
          type="text"
          style={{ ...inputStyle, marginBottom: "4px" }}
        ...
        <label style={labelTitleStyle}>FECHA DE DISEÑO:</label>
        <input
          type="date"
          style={{ ...inputStyle, marginBottom: "4px" }}
          value={fechaDiseno}
        ...
        <div style={cardStyle}>
          {/* CAMBIO DE TEXTO: Estructura a EJES */}
          <label style={{ ...cardTitleStyle, fontSize: "9.5px" }}>
          ...
          >
            <div style={{ flex: "1 1 30%" }}>
              <label style={{ fontSize: "9px" }}>N° Ejes:</label>
              <input
                type="number"
                style={inputStyle}
                value={numColumnasInput}
                onChange={(e) => setNumColumnasInput(e.target.value)}
              />
            </div>
            <div style={{ flex: "1 1 30%" }}>
              <label style={{ fontSize: "9px" }}>Inicio en:</label>
              <input
                type="number"
                style={inputStyle}
                value={inicioEjeNum}
                onChange={(e) => setInicioEjeNum(e.target.value)}
              />
            </div>
            <div style={{ flex: "1 1 30%" }}>
              ...
              <select
                ...
                value={calibreCanal}
                onChange={(e) => setCalibreCanal(e.target.value)}
              >
          ...
          <button
            onClick={duplicarEstructuraViga1}
            style={{
              ...btnStyle,
              width: "100%",
              marginTop: "8px",
          ...
          >
            📋 Copiar Diseño Eje 1 a Todos
          </button>
          <button
            onClick={() => setInvertirNumeracion(!invertirNumeracion)}
            style={{
              ...btnStyle,
              width: "100%",
              marginTop: "4px",
        ...
        >
          <label
            style={{ ...cardTitleStyle, color: "#1e293b", fontSize: "9.5px" }}
          >
            📐 MEDIDAS POR PUNTO
          </label>
          ...
          <div style={{ marginTop: "8px", marginBottom: "8px" }}>
            <select
              ...
              value={hitoSeleccionado || "viga-0"}
              onChange={(e) => setHitoSeleccionado(e.target.value)}
            >
              {hitosOrdenados.map((h) => (
                <option key={h.keyHash} value={h.keyHash}>
                  {h.label}
                </option>
              ))}
            </select>
          ...
          >
            ...
            <span style={{ fontSize: "11px", color: alertaDesarrollo.color }}>
              {desarrolloHitoActual} mm {alertaDesarrollo.texto}
            </span>
          </div>
          ...
          >
            <button
              onClick={duplicarMedidasEnTodosLosHitos}
              style={{
                ...btnStyle,
          ...
          >
            <button
              onClick={() => toggleCaraExtrema("P1")}
              style={{
                ...btnStyle,
                backgroundColor: tieneP1 ? "#ef4444" : "#1d4ed8",
                color: "#fff",
                border: "none",
                fontSize: "9px",
                fontWeight: "bold",
            ...
            >
              {tieneP1 ? "🗑️ Quitar Cara P1" : "➕ Devolver Cara P1"}
            </button>
            <button
              onClick={() => toggleCaraExtrema("ULTIMA")}
              style={{
                ...btnStyle,
                backgroundColor: tieneUltima ? "#b91c1c" : "#2563eb",
                color: "#fff",
                border: "none",
                fontSize: "9px",
                fontWeight: "bold",
            ...
            >
              {tieneUltima ? "🗑️ Quitar Última" : "➕ Devolver Última"}
            </button>
          </div>
            ...
            {plieguesHitoActual.map((p, idx) => (
              ...
              >
                ...
                <input
                  ...
                  value={p.longitud}
                  onChange={(e) =>
                    modificarPliegueDeHito(idx, "longitud", e.target.value)
                  }
                />
        ...
        >
          <label
            style={{ ...cardTitleStyle, color: "#d35400", fontSize: "9.5px" }}
          >
            Plantilla Base (Caras)
          </label>
          <button
          ...
          >
            ...
            <select
              style={{ ...inputMiniStyle, fontSize: "10px" }}
              value={colorInterior}
            ...
            <select
              style={{ ...inputMiniStyle, fontSize: "10px" }}
              value={ladoCubierta}
              onChange={(e) => setLadoCubierta(e.target.value)}
            >
            ...
            <input
              ...
              value={anchoAbertura}
              onChange={(e) => setAnchoAbertura(e.target.value)}
              placeholder="Ej. 530"
            />
            <label
              style={{
            ...
            >
              Abrir Aleta (Ángulo Libre):
            </label>
            <select
              style={{ ...inputMiniStyle, fontSize: "10px" }}
              value={ladoAbrir}
              onChange={(e) => setLadoAbrir(e.target.value)}
            >
          ...
          >
            {pliegues.map((p, idx) => (
              ...
              >
                ...
                <input
                  type="text"
                  style={{ ...inputMiniStyle, width: "25px", fontSize: "9px" }}
                  value={p.angulo}
                  onChange={(e) =>
                    modificarPliegue(idx, "angulo", e.target.value)
                  }
                />
                <input
                  type="text"
                  style={{ ...inputMiniStyle, width: "35px", fontSize: "9px" }}
                  value={p.longitud}
                  onChange={(e) =>
                    modificarPliegue(idx, "longitud", e.target.value)
                  }
                />
                <input
                  type="text"
                  style={{ ...inputMiniStyle, flex: 1, fontSize: "9px" }}
                  value={p.detalle}
                  onChange={(e) =>
                    modificarPliegue(idx, "detalle", e.target.value)
                  }
                />
                <button
                  onClick={() => eliminarPliegue(idx)}
                  style={{
                    border: "none",
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
            <div style={{ display: "flex", gap: "2px" }}>
              <input
                ...
                value={nuevoAngulo}
                placeholder="Ang (°)"
                onChange={(e) => setNuevoAngulo(e.target.value)}
              />
              <input
                type="number"
                style={{ ...inputMiniStyle, flex: 1 }}
            ...
            <button
              onClick={() => setInvertirTramos(!invertirTramos)}
              style={{
                ...btnStyle,
                width: "100%",
                marginTop: "2px",
        ...
        >
          ...
          >
            ...
            <div style={{ textAlign: "right" }}>
              ...
              >
                <span>
                  <strong>Calibre:</strong> {calibreCanal}
                </span>
                <span>
                ...
                <span>
                  <strong>Fecha:</strong>{" "}
                  {fechaDiseno || "____________________"}
                </span>
              </div>
            </div>
          </div>
          ...
          <div style={{ width: "100%", position: "relative" }}>
            ...
            >
                    ...
                    {geom.ejesTraslapos.map((eje, tIdx) => (
                      <g key={`eje-t-${tIdx}`}>
                        ...
                        <rect
                          ...
                          onContextMenu={(e) =>
                            abrirMenuTraslapo(e, colId, eje.index)
                          }
                        />
                      </g>
                    ))}
              ...
              {Object.keys(datosGeometria).map((colId) => (
                <g key={`sosco-${colId}`} style={{ pointerEvents: "none" }}>
                ...
                <g key={col.id}>
                  {col.esMuro ? (
                    <>
                      ...
                      {/* 💡 CAMBIO: Bajamos el inicio del muro a 'y={28}' y acortamos la altura a '92' para despejar el texto */}
                      <rect
                        x={col.x - 2.5}
                        y={28}
                        width="5"
                        height="92"
                        fill="url(#pattern-ladrillo)"
                        stroke="#94a3b8"
                        strokeWidth="1.5"
                      />
                      <rect
                        ...
                        fill="transparent"
                        style={{ cursor: "context-menu" }}
                        onContextMenu={(e) => abrirMenuColumna(e, col.id)}
                      />
                    </>
                  ) : (
                    <>
          ...
          {!menuFlotante.esTraslapo ? (
            <React.Fragment>
              <div style={menuHeaderStyle}>
                Eje{" "}
                {
                  columnasCalculadas.find(
                    (c) => c.id === menuFlotante.columnaId,
                  )?.numero
                }
              </div>

              <div style={seccionSeparadorStyle}>
                <span style={seccionTituloMiniStyle}>ESTRUCTURA DE MUROS</span>
                {(menuFlotante.columnaId === 0 ||
                  menuFlotante.columnaId === "muro-izq") && (
                  <button
                    onClick={() => {
                      setMuroIzquierdo(!muroIzquierdo);
                      setMenuFlotante({ ...menuFlotante, visible: false });
                    }}
                    style={{
                      ...btnStyle,
                ...
                {(menuFlotante.columnaId === numColumnas - 1 ||
                  menuFlotante.columnaId === "muro-der") && (
                  <button
                    onClick={() => {
                      setMuroDerecho(!muroDerecho);
                      setMenuFlotante({ ...menuFlotante, visible: false });
                    }}
                    style={{
                      ...btnStyle,
              ...
              <button
                onClick={() =>
                  agregarNuevoTraslapoCadena(menuFlotante.columnaId)
                }
                style={{
                  ...btnStyle,
                  width: "100%",
              ...
              {colActual.listaTraslapos?.length > 0 && (
                <button
                  onClick={() => {
                    if (
                      window.confirm("¿Seguro de remover el último traslapo?")
                    )
                      eliminarUltimoTraslapo(menuFlotante.columnaId);
                  }}
                  style={{
                    ...btnStyle,
                    width: "100%",
              ...
              {colActual.listaTraslapos?.length === 0 && (
                <React.Fragment>
                  <label style={itemMenuLabelStyle}>
                    <input
                      type="checkbox"
                      checked={colActual.unirDerecha}
                      onChange={(e) =>
                        actualizarPropiedadColumna(
                          menuFlotante.columnaId,
                          "unirDerecha",
                          e.target.checked,
                        )
                  ...
                  {colActual.unirDerecha && (
                    <div style={subSeccionStyle}>
                      ...
                      <input
                        ...
                        value={colActual.longitudDerecha}
                        onChange={(e) =>
                          actualizarPropiedadColumna(
                            menuFlotante.columnaId,
                            "longitudDerecha",
                ...
                <div style={{ display: "flex", gap: "3px" }}>
                  <div>
                    ...
                    <input
                      type="text"
                      style={inputMiniStyle}
                      value={colActual.planaIzquierda || ""}
                      onChange={(e) =>
                        actualizarPropiedadColumna(
                          menuFlotante.columnaId,
                          "planaIzquierda",
              ...
              <div style={seccionSeparadorStyle}>
                ...
                >
                  ...
                  <select
                    ...
                    value={colActual.diametroSosco || '4"'}
                    onChange={(e) =>
                      actualizarPropiedadColumna(
                        menuFlotante.columnaId,
                        "diametroSosco",
                ...
                <label style={itemMenuLabelStyle}>
                  <input
                    type="radio"
                    name="soscoGroup"
                    checked={!!colActual.soscoCentro}
                    onChange={() => setSoscoType("c")}
                  />{" "}
                  Sosco Central
                </label>
                <label style={itemMenuLabelStyle}>
                  <input
                    type="radio"
                    name="soscoGroup"
                    checked={!!colActual.soscoIzquierdo}
                    onChange={() => setSoscoType("i")}
                  />{" "}
                  Sosco Izquierdo
                </label>
                <label style={itemMenuLabelStyle}>
                  <input
                    type="radio"
                    name="soscoGroup"
                    checked={!!colActual.soscoDerecho}
                    onChange={() => setSoscoType("d")}
                  />{" "}
                  Sosco Derecho
                </label>
                <label style={itemMenuLabelStyle}>
                  <input
                    type="radio"
                    name="soscoGroup"
                    checked={!!colActual.dosSoscos}
                    onChange={() => setSoscoType("dos")}
                  />{" "}
                  Dos Soscos
                </label>
                <button
            ...
            <React.Fragment>
              ...
              <button
                onClick={() =>
                  eliminarTraslapoEspecifico(
                    menuFlotante.columnaId,
                    menuFlotante.traslapoIndex,
                  )
                }
                style={{
                  ...btnStyle,
              ...
              <div style={{ marginTop: "6px" }}>
                ...
                <input
                  type="text"
                  style={inputMiniStyle}
                  value={traslapoActual?.longitud || ""}
                  onChange={(e) =>
                    actualizarPropiedadTraslapo(
                      menuFlotante.columnaId,
                      menuFlotante.traslapoIndex,
                      "longitud",
                      e.target.value,
                ...
                <input
                  type="text"
                  style={inputMiniStyle}
                  value={traslapoActual?.pendiente || ""}
                  onChange={(e) =>
                    actualizarPropiedadTraslapo(
                      menuFlotante.columnaId,
                      menuFlotante.traslapoIndex,
              ...
              <div style={seccionSeparadorStyle}>
                ...
                <label style={itemMenuLabelStyle}>
                  <input
                    type="radio"
                    name="destinoGroup"
                    checked={traslapoActual?.conectarA === "traslapo"}
                    onChange={() =>
                      actualizarPropiedadTraslapo(
                        menuFlotante.columnaId,
                        menuFlotante.traslapoIndex,
                  ...
                  />{" "}
                  Encadenar otro Traslapo
                </label>
                <label style={itemMenuLabelStyle}>
                  <input
                    type="radio"
                    name="destinoGroup"
                    checked={traslapoActual?.conectarA === "columna"}
                    onChange={() =>
                      actualizarPropiedadTraslapo(
                        menuFlotante.columnaId,
                        menuFlotante.traslapoIndex,
              ...
              {traslapoActual?.conectarA === "columna" && (
                <div style={subSeccionStyle}>
                  ...
                  <input
                    type="text"
                    style={inputMiniStyle}
                    value={traslapoActual?.longitudCierre || ""}
                    onChange={(e) =>
                      actualizarPropiedadTraslapo(
                        menuFlotante.columnaId,
                        menuFlotante.traslapoIndex,
                  ...
                  <input
                    type="text"
                    style={inputMiniStyle}
                    value={traslapoActual?.pendienteCierre || ""}
                    onChange={(e) =>
                      actualizarPropiedadTraslapo(
                        menuFlotante.columnaId,
                        menuFlotante.traslapoIndex,
          ...
          <div style={modalBodyStyle}>
            ...
            >
              {listaProyectos.map((p) => (
                <div
                  key={p.id}
                  style={proyectoItemStyle}
                  onClick={() => cargarProyectoEspecifico(p)}
                >
...
const seccionSeparadorStyle = {
  borderTop: "1px solid #e2e8f0",
  paddingTop: "4px",
  marginTop: "6px",
};
const seccionTituloMiniStyle = {
  fontSize: "9px",
  fontWeight: "bold",
  color: "#64748b",
  display: "block",
